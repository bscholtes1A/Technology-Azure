...
import static org.eclipse.edc.test.system.TransferRuntimeConfiguration.CONSUMER_PARTICIPANT_ID;
...
import static org.eclipse.edc.test.system.TransferRuntimeConfiguration.PROVIDER_PARTICIPANT_ID;
...
public class BlobTransferIntegrationTest extends AbstractAzureBlobTest {
    ...
    void transferBlob_success() {
        ...
        // Write Key to vault
        consumer.getContext().getService(Vault.class).storeSecret(format("%s-key1", account2Name), account2Key);
        provider.getContext().getService(Vault.class).storeSecret(format("%s-key1", account1Name), account1Key);
